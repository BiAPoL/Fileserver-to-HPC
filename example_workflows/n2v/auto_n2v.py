...
def predict(data_dir: Path, model_dir: Path, target_dir: Path):
    ...
    jobs = []
    slurmfile = Path(__file__).parent / 'scripts' / 'n2v_predict.slurm'
    src_dir = str(Path(__file__).parent)
    for file in files:
        if not (data_dir / "denoised" / (file.name + "_N2V.tif")).exists():
            args = ['sbatch', '-o', str(data_dir / 'log' / (file.name + '_n2v.out')), str(slurmfile), str(file), str(target_dir), str(model_dir), src_dir]
...
if __name__ == "__main__":
    ...
    if not (data_dir / 'training').exists():
        ...
        assert model_data_dir.exists(), 'Need either a subdirectory called "training" containing training data for a new model or a subdirectory called "model" containing a trained model.'
        predict(data_dir=data_dir, model_dir=model_data_dir, target_dir=cache_dir)
    else:
        ...
        if model_data_dir.exists():
            if (training_data_dir / 'continue_training').exists():
                ...
                copytree(model_data_dir, model_dir)
        ...
        predict(data_dir=data_dir, model_dir=model_dir, target_dir=cache_dir)
